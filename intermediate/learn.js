...
(function () {
    ...
    var K_PROGRESS = 'tagalog-int-progress';   // { "1": {completed, visited, ts}, ... }
    var K_KNOWN    = 'tagalog-int-known';      // legacy flat map { "salamat": ts, ... } — migrated into SRS
    var K_SRS      = 'tagalog-int-srs';        // { "salamat": {box,due,reps}, ... }
    var K_JOURNAL  = 'tagalog-int-journal:';   // + page slug

    // Intermediate course: derive the lesson count from the vocab titles map so
    // the dashboard stays correct as B1/B2 lessons are added (no manual bumping).
