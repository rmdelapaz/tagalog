...
Output: tagalog_intermediate.apkg   (INTERMEDIATE B1-B2 deck — own IDs/filename)
"""
import json
import os
import re
...
# Distinct from the beginner deck so the two decks never collide in Anki.
MODEL_ID = 1607392947
DECK_ID = 2059400271
...
def main():
    ...
    for k in [str(i) for i in range(1, 25)]:
        for w in vocab[k]:
    ...
    model = genanki.Model(
        MODEL_ID,
        "Tagalog Intermediate (Tagalog/English)",
        fields=[
            {"name": "Tagalog"},
            {"name": "Pronunciation"},
            {"name": "English"},
...
""",
    ...
    deck = genanki.Deck(DECK_ID, "Tagalog Intermediate (B1–B2)")
    ...
    out = os.path.join(HERE, "tagalog_intermediate.apkg")
