...
Run:  python3 build_anki.py     (from the advanced folder; needs `genanki`)
Output: tagalog_advanced.apkg   (ADVANCED C1-C2 deck — own IDs/filename)
"""
import json
import os
import re
...
# Distinct from the beginner and intermediate decks so the three decks never collide in Anki.
MODEL_ID = 1738294613
DECK_ID = 2104558832
...
def main():
    ...
    model = genanki.Model(
        MODEL_ID,
        "Tagalog Advanced (Tagalog/English)",
        fields=[
            {"name": "Tagalog"},
            {"name": "Pronunciation"},
            {"name": "English"},
...
""",
    ...
    deck = genanki.Deck(DECK_ID, "Tagalog Advanced (C1–C2)")
    ...
    out = os.path.join(HERE, "tagalog_advanced.apkg")
